...
local layer, parent = torch.class('nn.LM_decoder', 'nn.Module')
...
function layer:sample_beam(imgs, opt)
  ...
  local batch_size, feat_dim = imgs[1]:size(1), imgs[1]:size(2)
  ...
  for k=1,batch_size do
    ...
    --TODO: copy state to beam_size
    local state = {}
    for ix, s in pairs(imgs) do
      state[ix] = s:expand(beam_size, feat_dim)
    end
